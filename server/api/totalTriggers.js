...
router.get('/totalTriggers', async (req, res) => {
    const { rows } = await db.query(
        'SELECT COUNT(id) AS recordedcount FROM stats',
    )
...
router.get('/totalTriggers/:guildid', async (req, res) => {
    ...
    const { rows } = await db.query(
        'SELECT COUNT(id) AS recordedcount FROM stats WHERE server_id = $1',
        [guildid],
    )
    rows[0].recordedcount = Number(rows[0].recordedcount)
    res.send(rows[0])
...
router.get('/totalservers', async (req, res) => {
    const { rows } = await db.query(
        'SELECT COUNT(server_id) AS nbservers FROM servers WHERE active = true',
    )
...
router.get('/totalusers', async (req, res) => {
    const { rows } = await db.query(
        'SELECT COUNT(DISTINCT author_id) AS uniqueusers FROM stats',
    )
