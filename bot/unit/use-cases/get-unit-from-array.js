module.exports = function makeGetUnitFromArray ({ unitsList, handleAliases, getUnitFromCode, makeNaval }) {
    return function getUnitFromArray(unitArray, replyData) {
        ...
        if (unitCode.length === 0 && navalUnitArray.length != 0) {
            if(currentHPArray.length !== 0)
                throw `You need to provide a unit inside the **\`${navalUnitArray[0]}\`**\nYou can see the full unit list with\`/units\`.`
            else {
                unitCode = ['wa']
