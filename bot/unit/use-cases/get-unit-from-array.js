module.exports = function makeGetUnitFromArray({
    unitsList,
    handleAliases,
    getUnitFromCode,
}) {
    return function getUnitFromArray(unitArray, replyData) {
        ...
        const navalAndModifiers = unitArray.filter(
            (value) => !unitKeys.includes(value.substring(0, 2).toLowerCase()),
        )
        const navalUnitCode = navalAndModifiers.filter(
            (value) =>
                value.includes('rf') ||
                value.includes('sc') ||
                value.includes('bo') ||
        ...
        const unitModifiers = navalAndModifiers.filter(
            (value) =>
                !(
                    value.includes('rf') ||
                    value.includes('sc') ||
        ...
        } else unitCode = unitCode.substring(0, 2).toLowerCase()

        const unit = getUnitFromCode(unitCode)

        if (navalUnitCode) unit.makeNaval(navalUnitCode)

        // s f
        if (unitModifiers.includes('d')) unit.setBonus(['d'], replyData)

        if (unitModifiers.includes('w')) {
            if (unitModifiers.includes('d'))
                unit.setBonus(['d', 'w'], replyData)
            else unit.setBonus(['w'], replyData)
        }

        if (unitModifiers.includes('b')) unit.toBoost(replyData)
        ...
        const newHP = Number(
            unitModifiers.filter((modifier) => numberRegex.test(modifier))[0],
        )

        if (newHP) unit.setHP(newHP, replyData)
