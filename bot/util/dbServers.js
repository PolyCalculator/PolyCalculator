    ...
    return new Promise((resolve, reject) => {
        this.serverExists(serverId)
            .then((serverExists) => {
                if (serverExists) {
                    const sql =
                        'UPDATE servers SET server_name = $1, active = true WHERE server_id = $2'
                    const values = [serverName, serverId]

                    pool.query(sql, values, (err, res) => {
                        if (err) {
                            reject(
                                `${err.message}. Ping an @**admin** if you need help!`,
                            )
                        } else {
                            resolve(
                                `**${serverName}** (${serverId}) reinvited the bot and was updated!`,
                            )
                        }
                    })
                } else {
                    const sql =
                        'INSERT INTO servers (server_id, server_name, active) VALUES ($1, $2, true)'
                    ...
                    pool.query(sql, values, (err, res) => {
                        if (err) {
                            ...
                            resolve(
                                `**PolyCalculator** was added to **${serverName}** (${serverId})!`,
                            )
                        }
                    })
                }
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.query(sql, values, (err, res) => {
            if (err) {
                ...
                resolve(
                    `The bot was removed from __**${serverName}**__ (${serverId}) :frowning:...`,
                )
            }
        })
    })
    ...
    return new Promise((resolve, reject) => {
        ...
        pool.query(sql, values, (err, res) => {
            if (err) {
                ...
                if (res.rows[0] === undefined) resolve(false)
                else resolve(true)
            }
        })
    })
