...
module.exports = {
  ...
  execute: async function(message, user, replyData/*, dbData*/) {
    ...
    try {
      ...
      replyData.discord.description = `${(user) ? `These are ${user}'s stats for this bot in ${message.guild.name}` : `These are your stats for this bot in ${message.guild.name}`}`
      replyData.discord.fields.push({ name: 'Local count', value: `${(localUserStats.rows.length > 0) ? localUserStats.rows[0].count : 'Never here'}` })
      replyData.discord.fields.push({ name: 'Global count', value: `${(globalUserStats.rows.length > 0) ? globalUserStats.rows[0].count : 'Never used it at all :scream:'}` })
