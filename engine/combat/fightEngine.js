...
import deadText from './deadtexts.js'
import { attackerCalc, defenderCalc } from '../effects.js'
import {
...
export function optim(attackers, defender, replyData, target) {
...
export function calc(attackers, defender, replyData) {
    // Validate: units with no attack stat can only be used with explode modifiers
    for (const attacker of attackers) {
        if (attacker.canAttack === false && !attacker.exploding) {
            if (attacker.attackExplode) {
...
export function bulk(attacker, defender, replyData) {
    const aforce =
...
export function provideDefHP(attacker, defender, replyData) {
    let aforce =
        (attacker.iAtt() * attacker.iCurrentHp() * 100n) / attacker.iMaxHp()
    const dforce =
        (defender.iDef() * defender.iCurrentHp() * 100n) / defender.iMaxHp()
...
export function provideAttHP(attacker, defender, replyData) {
    const aforce =
        (attacker.iAtt() * attacker.iCurrentHp() * 100n) / attacker.iMaxHp()
    let dforce =
        (defender.iDef() * defender.iCurrentHp() * 100n) / defender.iMaxHp()
