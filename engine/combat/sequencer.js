import { poison, freeze, convert } from '../effects.js'
import { attackerCalc, defenderCalc } from '../effects.js'

export function generateArraySequences(length) {
    const array = []
...
export function generateSequences(xs) {
    const ret = []
...
export function multicombat(attackers, defender, sequence) {
...
export function evaluateWithTarget(bestSolution, newSolution, target) {
...
export function evaluate(bestSolution, newSolution) {
    if (newSolution.defenderHP < bestSolution.defenderHP) return true
    else {
